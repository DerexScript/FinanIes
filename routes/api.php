...
$router->group(['prefix' => '/api/v1/'], function () use ($router) {
    ...
    $router->group(['prefix' => 'release', 'middleware' => ['auth:api', 'access.control.list']], function () use ($router) {
        ...
        $router->get('/{release}', ['uses' => 'ReleaseController@get', 'as' => 'release.get']);
    ...
    $router->group(['prefix' => 'release-group', 'middleware' => ['auth:api', 'access.control.list']], function () use ($router) {
        $router->get('/', ['uses' => 'ReleaseGroupController@index', 'as' => 'releaseGroup.index']);
        $router->get('/{releaseGroup}', ['uses' => 'ReleaseGroupController@get', 'as' => 'releaseGroup.get']);
        $router->post('/', ['uses' => 'ReleaseGroupController@store', 'as' => 'releaseGroup.store']);
        $router->put('/{releaseGroup}', ['uses' => 'ReleaseGroupController@update', 'as' => 'releaseGroup.update']);
        $router->delete('/{releaseGroup}', ['uses' => 'ReleaseGroupController@destroy', 'as' => 'releaseGroup.destroy']);
    });
